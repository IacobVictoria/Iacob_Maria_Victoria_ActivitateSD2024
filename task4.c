...
struct Imobil* citireFisierCreareVector(int* dim) {
    ...
    while (1) {
        if (k >= cap) {
            ...
            //Aceasta se face cu ajutorul funcției realloc. Realocarea nu doar mărește spațiul de memorie disponibil, ci și copiază datele existente în noul spațiu de memorie.
            vector = temp;
        }
...
//matrice dezalocare
struct Imobil copieImobil(struct Imobil c) {
    struct Imobil nou;
    nou.cod = c.cod;
    nou.adresa = (char*)malloc(strlen(c.adresa) + 1);
    strcpy(nou.adresa, c.adresa);
    nou.nrEtaje = c.nrEtaje;
    nou.preturi = (float*)malloc(sizeof(float) * c.nrEtaje);
    for (int i = 0; i < c.nrEtaje; i++) {
        nou.preturi[i] = c.preturi[i];
    }
    return nou;

}
int nrMinimEtaje(struct Imobil* v, int dim) {
    int min = v[0].nrEtaje;
    for (int i = 1; i < dim; i++) {
        if (v[i].nrEtaje < min) {
            min = v[i].nrEtaje;
...
int nrMaximEtaje(struct Imobil* v, int dim) {
    int max = v[0].nrEtaje;
    for (int i = 1; i < dim; i++) {
        if (v[i].nrEtaje > max) {
            max = v[i].nrEtaje;
...
//afisare matrice
void afisareMatrice(int nrLinii, int* nrElementeLinie, struct Imobil** matrice) {
    for (int i = 0; i < nrLinii; i++) {
        for (int j = 0; j < nrElementeLinie[i]; j++) {
            afiseazaImobil(matrice[i][j]);
        }
        printf("\n");
    }
}
    ...
    //Creati o functie care sa copieze (deep copy) elementele din vector intr-o matrice alocată dinamic.dupa un criteriu, dupa nr de etaje
struct Imobil** creareMatricePeNrEtaje(struct Imobil* vector, int dim, int* nrLinii, int** nrElementeLinie) {
    int etajeMin = nrMinimEtaje(vector, dim);
    int etajemax = nrMaximEtaje(vector, dim);
    *nrLinii = 2;
    *nrElementeLinie = (int*)malloc(sizeof(int) * (*nrLinii));
    struct Imobil** matrice = (struct Imobil**)malloc(sizeof(struct Imobil*) * (*nrLinii));
    for (int i = 0; i < (*nrLinii); i++) {
        (*nrElementeLinie)[i] = 0;                     //INITIALIZARE
        matrice[i] = NULL;
    }
    for (int i = 0; i < dim; i++) {
        int l = 0;
        if (vector[i].nrEtaje == etajeMin) {
            l = 0;
        }
        else if (vector[i].nrEtaje > etajeMin) {
            l = 1;
        }

        struct Imobil* aux = (struct Imobil*)malloc(sizeof(struct Imobil) * ((*nrElementeLinie)[l] + 1));
        if (matrice[l] != NULL) {
            for (int i = 0; i < (*nrElementeLinie)[l]; i++) {
                aux[i] = matrice[l][i];
        ...
        aux[(*nrElementeLinie)[l]] = copieImobil(vector[i]);
        matrice[l] = aux;
        ((*nrElementeLinie)[l])++;
...
//dezalocare matrice
void dezalocareMatrice(struct Imobil*** matrice, int* nrLinii, int** nrElementeLinie) {
    for (int i = 0; i < *nrLinii; i++) {
        for (int j = 0; j < (*nrElementeLinie)[i]; j++) {
            free((*matrice)[i][j].adresa);
...
void sortareCrescatoare(struct Imobil ** matrice, int nrLinii, int* nrElem) {
    for (int i = 0; i < nrLinii - 1; i++) {
        for (int j = i + 1; j < nrLinii; j++) {
            if (nrElem[i] > nrElem[j]) {
                int aux = nrElem[i];
                nrElem[i] = nrElem[j];
                nrElem[j] = aux;
                struct Imobil* auxPointer = matrice[i];
                matrice[i] = matrice[j];
                matrice[i] = auxPointer;
            }
        }
    }
}
int main() {
    ...
    struct Imobil** imobile = creareMatricePeNrEtaje(vectorF, dimenV, &nrLinii, &nrElementeLinie);

    afisareMatrice(nrLinii, nrElementeLinie, imobile);
    return 0;
}
